package classes {
	...
	public class RagDoll {
		...
		public function RagDoll(m_world:b2World ) {
			...
			var startX:Number = 400 + Math.random() * 300;
			...
			head = m_world.CreateDynamicBody(bd);
			...
			// Leg
			box.SetAsBox(8 / m_physScale, 15 / m_physScale);
			box.density = 0.5;
			box.friction = 0.4;
			box.restitution = 0.1;
			bd = new b2BodyDef();
			// L
			bd.position.Set((startX - 10) / m_physScale, (startY + 75) / m_physScale);
			left_leg = m_world.CreateDynamicBody(bd);
